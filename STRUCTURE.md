...
| `src/checkpoint1_analysis.py` | Generates `reports/checkpoint1_summary.md` from cached SCDB + bulk-fetch log + parquet — Phase 1 detailed report. |
...
| `reports/JusticeCast_Pitch.pdf` | Phase 7 pitch deck PDF (11 slides). Generated externally by the PowerPoint Claude extension from the asset bundle in `reports/deck_assets/`. |
| `reports/checkpoint1_summary.md` | Auto-generated Phase 1 detailed report (pipeline stages, Justice coverage, label distribution, multi-audio cases). Produced by `src/checkpoint1_analysis.py`. |
...
25+ CSV files spanning Phases 1–5. Notebook reads from these at render time; we don't re-run modeling sweeps to re-knit the notebook (Non-Negotiable #7).
